...
(defun pivotal-add-story-callback (status)
  (let* ((json  (pivotal-get-json-from-current-buffer))
         ;; (story (pivotal-format-story (car json)))
         )
    ;; (with-current-buffer (get-buffer-create "*pivotal-iteration*")
    ;;   (pivotal-append-story-to-current-project story))
    json))
...
(defun pivotal-get-json-from-current-buffer ()
  (re-search-forward "\r?\n\r?\n")
  (let ((json (json-read-from-string (buffer-substring-no-properties (point) (point-max)))))
    (kill-buffer)
    json))
